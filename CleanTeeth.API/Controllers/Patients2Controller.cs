...
namespace CleanTeeth.API.Controllers
{
    ...
    public class Patients2Controller : ControllerBase
    {
        ...
        {            
            var result = await _mediator.Send(new CreateCommand<Patient> { Entity = entity });
            return Ok(result);
        }
        ...
        {
            await _mediator.Send(new UpdateCommand<Patient> { Entity = entity });
            return NoContent();
        }
        ...
        public async Task<IActionResult> Delete(Guid id)
        {
            try
            {
                await _mediator.Send(new DeleteCommand<Patient> { Id = id });
                return NoContent();
            }
